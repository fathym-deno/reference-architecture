/**
 * `ExtractExact<T, Tag>` is a TypeScript utility type that constructs a new type by
 * extracting from `Tag` those types that exactly match `T` from the union.
 *
 ...
 * @template Tag - The union of types from which an exact match will be extracted.
 *
 * @example
 ...
 */
export type ExtractExact<T, Tag> = Tag extends T ? Extract<Tag, T> : never;
