...
export const CLIConfig = z.object({
  Name: z.string()
    ...
    .describe(
      "A user-facing, friendly name for the CLI. Shown in help output and documentation headers.",
    ),

  Tokens: z.array(z.string())
    .min(1, "At least one CLI token is required.")
    .describe(
      'All valid CLI tokens (aliases) that can invoke this tool. Examples: ["openindustrial", "oi"]',
    ),

  Version: z.string()
    .min(1, "CLI version is required.")
    .describe(
      "The current version of the CLI. Used in logs, help, and update flows.",
    ),
  ...
  Commands: z.string()
    ...
    .default("./commands")
    .describe(
      'Relative or absolute path to the root folder of command modules. Defaults to "./commands".',
    ),
});
