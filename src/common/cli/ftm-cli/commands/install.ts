import { join, dirname, z } from '../../.deps.ts';
...
export const InstallFlagsSchema = z.object({
  ...
  config: z.string().optional().describe('Path to .cli.json (default: ./.cli.json)'),
});
...
export default Command('install', 'Install a compiled CLI binary to your system')
  ...
  .Run(async ({ Params, Log, Services }) => {
    ...
    const targetBinaryPath = join(await InstallDFS.ResolvePath('.'), binaryName);
